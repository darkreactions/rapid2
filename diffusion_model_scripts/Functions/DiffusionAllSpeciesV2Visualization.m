function [t,htotal,ctotal,columnheight,totalmolsdcm,totalvolfrac,bincentotal,concdcm,concfah,concorg,concinorg,concsolv,h,volfracoi,visualization] = DiffusionAllSpeciesV2Visualization(columnheight,radius,molsdcm0,bins,dt,duration,d,mva,corg,cinorg,kin,kout,startingmolsdcm,totalmolssolvent,totalmolsinorg,totalmolsorg,totalmolsFAH,htov,solventord,organicord,solventconv,organicconv,antiind,solvind,orgind,FAHind,inorgind,vtotal,xtalheight,xtalindex,genfig)
    ...
    cooldown = 1;
    genfig = genfig; % 0 is no, 1 is yes
 ...
 %%
    for mtime = 1:length(t)
        ...
        if mtime == toi && genfig == 1
            ...
            set(gca,'XTick',[])
            title("Column Visualization of Model for DCM Concentration",'FontWeight','bold')
        end
    end
    volfracoi = totalvolfrac(:,toi);
end